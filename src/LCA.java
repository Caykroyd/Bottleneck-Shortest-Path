...
public final class LCA {
	...
	//Tarjan's algorithm. We used a array of LinkedList with each position in the array representing a node. 
	// This array was preprocessed, then we go through the queries just one time and the complexity is simply O( n ).
	static void TarjanLCA(Tree t, Node u, UnionFind U, boolean[] visited, ArrayList<LinkedList<Query>> q){
		u.LCA = u;
		
		for(Node child : u.children){
			
			TarjanLCA(t,child,U, visited, q);
			U.union(u.id, child.id);
			t.node[U.find(u.id)].LCA = u;
			
		}
		
		visited[u.id] = true;
		
		for(Query s : q.get(u.id)){
			
			if(visited[s.v.id] == true){
				Node k = t.node[U.find(s.v.id)].LCA;
				s.maxNoise = k.id;
